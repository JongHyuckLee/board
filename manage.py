...
@app.route('/<regex("[\S]{0,12}"):dirs>/<regex("[\S]{0,12}"):route>/')
...
@app.route('/<regex("[\S]{0,12}"):dirs>/<regex("[\S]{0,12}"):route>/<regex("[\S]{0,12}"):param>')
def mapping_routes_and_param(dirs, route, param):
    directories = [os.path.basename(directory) for directory in glob.glob('%s/%s/*' % (app.root_path, 'routes'))
                   if os.path.isdir(directory)]
    if dirs in directories:
        module_from = 'routes.%s.%s_route' % (dirs, dirs)
        ...
        return getattr(module, route)(param)
    else:
        abort(404)
